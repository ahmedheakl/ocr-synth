...
from synthetic_data_generation.chart_rendering.src.charts_generation.PieChartMatplotlibRenderer import PieChartMatplotlibRenderer
from synthetic_data_generation.chart_rendering.src.charts_generation.PieChartPyechartsRenderer import PieChartPyechartsRenderer
from synthetic_data_generation.chart_rendering.src.charts_generation.PieChartSeabornRenderer import PieChartSeabornRenderer
from synthetic_data_generation.chart_rendering.src.extract_data.utils import *

from docling_core.types.doc.document import ChartSlice, PicturePieChartData, PictureTabularChartData
...
class Pie(ChartAdapter):
    def __init__(self):
        print('loading the pie factory :)')
        #TODO: fix problem with Pyecharts and javascript error: echarts is not defined
        self._render = random.choice([PieChartMatplotlibRenderer(), PieChartSeabornRenderer()])
        if isinstance(self._render, PieChartMatplotlibRenderer):
            print(f'cooking a Pie Render with Matplotlib')
        elif isinstance(self._render, PieChartSeabornRenderer):
            print(f'cooking a Bar Render with Seaborn')
        elif isinstance(self._render, PieChartPyechartsRenderer):
            print(f'cooking a Bar Render with Pyechart')
        else:
            print('strange flavour of pie initialization')
        
        self.table = None

    def convert_docling_table_to_data(self, docling_table):
        '''This function transform a docling table item into a dictionary
           with a format aligned with the one expected from the Pie chart generator
           The conversion is table -> barchart -> piechart'''
        try:
        ...
        #Now we transfor the bar chart into a pie Chart, we choose randomly among the charts we calculated
        self.table_index = random.randint(0, len(formated_tables)-1)
        json_table = formated_tables[self.table_index]  
        labels = list(json_table["values"].keys())
        values = list(json_table["values"].values())

        values = [float(v) for v in values]
        total = sum(values)
        values = [round(x / total * 100, 2) for x in values]

        formated_table = {
            "title": json_table["title"],
            "source": json_table["source"],
            "x_title": json_table["x_title"],
            "y_title": json_table["y_title"],
            "values": {},
        }
        for label, value in zip(labels, values):
            formated_table["values"][label] = f"{value}%"
    ...
    def convert_chart_data_to_docling(self, chart_data):
        ...
        for label, value in values.items():
            #docling expect the value without the %
            value = float(value[0:-1])
            slices.append(ChartSlice(label=label, value=value))
        return PicturePieChartData(title=title,  slices=slices)
    ...
    def convert_chart_data_to_docling_updated(self, chart_data):
        ...
        # cells_by_row = [[cells[row * num_cols + col] for row in range(num_rows)] for col in range(num_cols)]
        # cells_bars = cells_by_row[0] + cells_by_row[self.table_index]
        cell_list = []
        values = chart_data['values']
        ...
        annotation = PictureClassificationData(provenance='synthetic', predicted_classes=[PictureClassificationClass(class_name='pie_chart', confidence=1.0)])
