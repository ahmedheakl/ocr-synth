...
class LayoutSettings:
    ...
    def is_persian(self):
        ...
        return self._language.lower() in ["persian", "farsi"]

    def is_rtl_language(self):
        """Check if the language is right-to-left"""
        return self._language.lower() in ["arabic", "hebrew", "urdu", "persian", "farsi"]
    ...
    def _configure(self, template: dict):
        ...
        # Updated font style resolution to handle Persian fonts
        style_name = layout_style.get(
            TemplateKeys.FONT_STYLE,
            AvailableFontStyles.get_default_font_for_language(self._language)
        )
    ...
    def _resolve_font_size(self, layout_style: dict) -> int:
        ...
        # Persian fonts work well with slightly larger sizes for better readability
        if self.is_persian() and font_size < AvailableFontSizes.SMALL_INT:
            return AvailableFontSizes.SMALL_INT
    ...
    # Add Persian-specific layout adjustments:
    def get_persian_line_spacing_multiplier(self) -> float:
        """Persian text may need more line spacing for certain fonts"""
        if self.is_persian():
            return 1.2  # 20% more line spacing for Persian
        return 1.0

    def get_persian_text_width_adjustment(self) -> float:
        """Persian may need slightly adjusted text width for better readability"""
        if self.is_persian():
            return 0.97  # 3% reduction in text width for Persian
        return 1.0

    def get_persian_paragraph_spacing(self) -> float:
        """Persian-specific paragraph spacing adjustments"""
        if self.is_persian():
            return 1.5  # Slightly more space between paragraphs
        return 1.0

    def requires_persian_number_formatting(self) -> bool:
        """Check if Persian number formatting should be applied"""
        return self.is_persian()

    def get_persian_calendar_support(self) -> bool:
        """Check if Persian calendar support should be enabled"""
        return self.is_persian()

    def get_rtl_text_alignment(self) -> str:
        """Get appropriate text alignment for RTL languages"""
        if self.is_rtl_language():
            return "right"  # RTL languages typically align right
        return "left"  # LTR languages align left

    def get_language_specific_line_spacing(self) -> float:
        """Get line spacing multiplier based on language"""
        if self.is_urdu():
            return self.get_urdu_line_spacing_multiplier()
        elif self.is_persian():
            return self.get_persian_line_spacing_multiplier()
        return 1.0  # Default spacing

    def get_language_specific_text_width_adjustment(self) -> float:
        """Get text width adjustment based on language"""
        if self.is_urdu():
            return self.get_urdu_text_width_adjustment()
        elif self.is_persian():
            return self.get_persian_text_width_adjustment()
        return 1.0  # No adjustment for other languages