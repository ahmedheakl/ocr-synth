...
class PackageExtender:
    ...
    def add_base_packages(self, doc: Document):
        ...
        if language == "arabic":
            ...
            if font_name == "random":
                font_name = random.choice(list(supported_fonts.keys()))
            self.set_font_name = supported_fonts.get(font_name, "Vazir")
        ...
        if language == "arabic":
            ...
            self.add_persian_support_packages(doc)
    ...
    def add_persian_support_packages(self, doc: Document):
        """Add Persian language support packages"""
        doc.packages.append(Package("fontspec"))
        doc.packages.append(Package("multirow"))
        doc.packages.append(Package("polyglossia"))
        doc.packages.append(Package("ragged2e"))
        ...
        doc.preamble.append(NoEscape(r"\renewcommand{\arraystretch}{1.3}"))
        doc.preamble.append(NoEscape(r"\setmainlanguage{farsi}"))  # Use 'farsi' for Persian in polyglossia
        doc.preamble.append(NoEscape(r"\setotherlanguage{english}"))
        
        # Set up Persian font with proper Arabic script support
        doc.preamble.append(NoEscape(
            fr"\newfontfamily\persianfont[Script=Arabic, Language=Persian, Direction=RTL, FallbackFonts={{Vazir, Noto Sans Arabic, Amiri, Noto Color Emoji, Symbola}}]{{{self.set_font_name}}}"
        ))
        
        # Also set up as \farsifont for polyglossia compatibility
        doc.preamble.append(NoEscape(
            fr"\newfontfamily\farsifont[Script=Arabic, Language=Persian, Direction=RTL, FallbackFonts={{Vazir, Noto Sans Arabic, Amiri, Noto Color Emoji, Symbola}}]{{{self.set_font_name}}}"
        ))
        
        # LuaTeX native RTL support for Persian
        doc.preamble.append(NoEscape(r"\def\setRTL{\pardir TRT \textdir TRT}"))
        doc.preamble.append(NoEscape(r"\def\setLTR{\pardir TLT \textdir TLT}"))
        
        # Set Persian-specific typography settings
        doc.preamble.append(NoEscape(r"\setRTL"))
        doc.preamble.append(NoEscape(r"\persianfont"))
        
        # Persian-specific line breaking and spacing
        doc.preamble.append(NoEscape(r"\tolerance=2000"))  # Higher tolerance for Persian
        doc.preamble.append(NoEscape(r"\emergencystretch=4em"))
        doc.preamble.append(NoEscape(r"\hyphenpenalty=10000"))  # Avoid hyphenation in Persian
        
        # Persian punctuation and number handling
        doc.preamble.append(NoEscape(r"\def\persiannumbers#1{\bgroup\persianfont\textdir TRT #1\egroup}"))
        
        # Persian-specific formatting
        doc.preamble.append(NoEscape(r"\def\persiandecimal#1{\bgroup\persianfont#1\egroup}"))  # For decimal numbers
        
        # Handle Persian calendar and date formatting if needed
        doc.preamble.append(NoEscape(r"\def\persiandate#1{\bgroup\persianfont\textdir TRT #1\egroup}"))
